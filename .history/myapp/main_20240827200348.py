...
import os
from encrypt_file import select_and_encrypt_file
...
theme_button.grid(row=0, column=1, sticky="ne")
...
text_frame = ttk.Frame(frame_input)
text_frame.grid(row=0, column=1, padx=5, pady=5, sticky="ew")

entry_text = Text(text_frame, width=30, height=3, wrap="word")
entry_text.pack(side="left", fill="both", expand=False)
...
strength_label.grid(row=0, column=1, sticky="w")
...
encrypt_file_btn = ttk.Button(app, text="Encrypt File", command=select_and_encrypt_file)
encrypt_file_btn.grid(row=0, column=1)
