...
from tkinter import Text, messagebox, Scrollbar, filedialog, Toplevel
...
def show_formula(algorithm):
    ...
    formula_label = ttk.Label(formula_window, text=get_formula(algorithm), wraplength=400, padding=10)
    formula_label.pack(padx=10, pady=10)
...
def get_formula(algorithm):
    formulas = {
        ...
        "XOR": "E_k(x) = x XOR k\nD_k(x) = x XOR k",
        "AES": "AES uses a combination of permutations and substitutions.",
        "RSA": "E_k(x) = x^e mod n\nD_k(x) = x^d mod n",
...
toggle_mode_button = ttk.Button(frame_actions, text="Toggle Mode", command=toggle_mode).grid(row=0, column=1, padx=5, pady=5)
copy_button = ttk.Button(frame_actions, text="Copy to Clipboard", command=copy_to_clipboard).grid(row=0, column=2, padx=5, pady=5)
save_button = ttk.Button(frame_actions, text="Save to File", command=save_to_file).grid(row=0, column=3, padx=5, pady=5)
follow_button = ttk.Button(frame_actions, text="Follow me", command=open_facebook).grid(row=0, column=4, padx=5, pady=5)


# Frame for displaying result
frame_result = ttk.Frame(app, padx=10, pady=10)
frame_result.grid(row=3, column=0, sticky="nsew")

result_label = ttk.Label(frame_result, text="Result:").grid(row=0, column=0, sticky="w")
