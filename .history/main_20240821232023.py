...
def toggle_theme():
    ...
    if current_bg == "#EDF7F6":
        ...
        entry_text.config(bg="gray", fg="#EDF7F6")
        entry_key.config(bg="gray", fg="#EDF7F6")
        result_display.config(bg="gray", fg="#EDF7F6")
    else:
...
def create_round_button(canvas, text, command, x, y, width, height, bg, fg, hover_bg, font):
    button = canvas.create_oval(x, y, x + width, y + height, outline=bg, fill=bg)
    label = canvas.create_text(x + width / 2, y + height / 2, text=text, fill=fg, font=font)
    
    def on_enter(event):
        canvas.itemconfig(button, fill=hover_bg)
    
    def on_leave(event):
        canvas.itemconfig(button, fill=bg)
...
# Canvas for custom buttons
canvas = tk.Canvas(frame_actions, bg='white', highlightthickness=0)
canvas.grid(row=0, column=0, padx=5, pady=5, columnspan=4)

# Create rounded buttons on the canvas
create_round_button(canvas, "Encrypt", process_text, 10, 10, 100, 50, "blue", "white", "lightblue", ("Helvetica", 12))
create_round_button(canvas, "Toggle Mode", toggle_mode, 120, 10, 100, 50, "green", "white", "lightgreen", ("Helvetica", 12))
create_round_button(canvas, "Copy to Clipboard", copy_to_clipboard, 230, 10, 150, 50, "orange", "white", "lightorange", ("Helvetica", 12))
create_round_button(canvas, "Save to File", save_to_file, 390, 10, 100, 50, "purple", "white", "lightpurple", ("Helvetica", 12))
create_round_button(canvas, "Facebook", open_facebook, 500, 10, 100, 50, "#4267B2", "white", "#3b5998", ("Helvetica", 12))
