...
def update_font_size(size):
    result_display.config(font=("Helvetica", int(size)))
...
font_scale = tk.Scale(app, from_=8, to=32, orient=tk.HORIZONTAL, label="Font Size", command=update_font_size)
font_scale.set(12) # Default font size is 12
font_scale.grid(row=4, column=0, padx=10, pady=10, sticky="w")
...
tk.Button(frame_actions, text="Follow me", command=open_facebook).grid(row=0, column=4, padx=5, pady=5)
...
result_display = Text(frame_result, height=10, width=50, wrap=tk.WORD)
