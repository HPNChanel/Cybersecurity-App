...
def check_password_strength(password):
    ...
    if len(password) < 8:
        ...
        strength_bar.config(value=50)
    elif not any(char in string.punctuation for char in password):
        strength_label.config(text="Strong", foreground="yellow")
        strength_bar.config(value=70)
    else:
        strength_label.config(text="Very Strong", foreground="green")
        strength_bar.config(value=100)

def randomize_password():
    length = 12
    password = ''.join(random.choices(string.ascii_letters + string.digits + string.punctuation, k=length))
    entry_text.delete(0, tk.END)
    entry_text.insert(0, password)
    app.clipboard_clear()
    app.clipboard_append(password)
    messagebox.showinfo("Password", "Password has been generated and copied to clipboard!")
...
entry_text = ttk.Entry(frame_input, width=40)  # Đặt kích thước cố định cho Entry
...
entry_key = ttk.Entry(frame_input, width=40)  # Đặt kích thước cố định cho Entry
...
# Password label
password_label = ttk.Label(frame_input, text="Password").grid(row=2, column=0, sticky="w")
entry_password = ttk.Entry(frame_input, width=50, show="*")
...
strength_label = ttk.Label(frame_input, text="")
...
strength_bar.grid(row=3, column=1, padx=5, pady=5, sticky="e")

random_password_button = ttk.Button(frame_input, text="Randomize Passwood", command=randomize_password)
random_password_button.grid(row=4, column=1, padx=5, pady=5, sticky="e")
